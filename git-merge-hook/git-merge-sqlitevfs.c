...
int main(int argc, char *argv[]) {
    ...
    system("git ls-files | grep '/pages/size.meta' > /tmp/gitvfs_dbs.txt");

    FILE *f = fopen("/tmp/gitvfs_dbs.txt", "r");
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *pages_ptr = '\0';
        ...
        system("rm -rf /tmp/gitvfs_base_db /tmp/gitvfs_local_db /tmp/gitvfs_remote_db");
        system("mkdir -p /tmp/gitvfs_base_db /tmp/gitvfs_local_db /tmp/gitvfs_remote_db");

        char cmd_extract[1024];
        snprintf(cmd_extract, sizeof(cmd_extract), "git archive %s \"%s/pages/\" | tar -x -C /tmp/gitvfs_base_db 2>/dev/null", base_commit, base_dir);
        system(cmd_extract);
        snprintf(cmd_extract, sizeof(cmd_extract), "git archive %s \"%s/pages/\" | tar -x -C /tmp/gitvfs_local_db 2>/dev/null", local_commit, base_dir);
        system(cmd_extract);
        snprintf(cmd_extract, sizeof(cmd_extract), "git archive %s \"%s/pages/\" | tar -x -C /tmp/gitvfs_remote_db 2>/dev/null", remote_commit, base_dir);
        system(cmd_extract);
        ...
        snprintf(local_db_path, sizeof(local_db_path), "/tmp/gitvfs_local_db/%s", base_dir);
        ...
        snprintf(attach_base, sizeof(attach_base), "ATTACH DATABASE '/tmp/gitvfs_base_db/%s' AS ancestor;", base_dir);
        sqlite3_exec(db_local, attach_base, NULL, 0, NULL);

        char attach_remote[1024];
        snprintf(attach_remote, sizeof(attach_remote), "ATTACH DATABASE '/tmp/gitvfs_remote_db/%s' AS other;", base_dir);
        ...
        char cmd_cp[1024];
        snprintf(cmd_cp, sizeof(cmd_cp), "rm -rf \"%s/pages\" && cp -r \"/tmp/gitvfs_local_db/%s/pages\" \"%s/pages\"", base_dir, base_dir, base_dir);
        system(cmd_cp);
